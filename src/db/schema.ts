...
export const cases = pgTable(
  "cases",
  {
    ...
    citizenActionId: text("citizen_action_id"),
    retryCount: integer("retry_count").notNull().default(0),
    lastVerifiedAt: timestamp("last_verified_at"),
    isDemo: boolean("is_demo").notNull().default(true),
    pendingConfirmation: text("pending_confirmation"),
    intakeLanguage: text("intake_language"),
    registrationNumber: text("registration_number"),
    lastPaymentDetails: jsonb("last_payment_details"),
    dispute: jsonb("dispute"),
    journeyId: text("journey_id"),
    journeyStep: integer("journey_step").notNull().default(0),
    createdAt: timestamp("created_at").defaultNow(),
...
export const caseEvents = pgTable(
  "case_events",
  {
    ...
    humanLabel: jsonb("human_label").notNull(),
